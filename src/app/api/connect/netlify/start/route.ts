...
export async function GET(request: Request) {
  ...
  if (!clientId) return NextResponse.redirect(new URL('/connect/netlify?oauth=error&reason=missing_client_id', baseUrl));
  ...
  authUrl.searchParams.set('redirect_uri', new URL('/api/connect/netlify/callback', baseUrl).toString());
