...
export async function GET(request: Request) {
  ...
  if (!clientId || !clientSecret) return NextResponse.redirect(new URL('/connect/clickup?oauth=error&reason=missing_env', url.origin));
  const cookieStore = await cookies();
  const expectedState = cookieStore.get('clickup_oauth_state')?.value;
  if (!expectedState || expectedState !== state || !code) return NextResponse.redirect(new URL('/connect/clickup?oauth=error&reason=invalid_state', url.origin));
  ...
  if (!tokenResp.ok) return NextResponse.redirect(new URL('/connect/clickup?oauth=error&reason=token_exchange_failed', url.origin));
  ...
  return NextResponse.redirect(new URL('/connect/clickup?oauth=success', url.origin));
}
