...
export async function GET(request: Request) {
  ...
  if (!clientId || !clientSecret) return NextResponse.redirect(new URL('/connect/canva?oauth=error&reason=missing_env', appBaseUrl()));
  ...
  if (!expectedState || expectedState !== state || !code || !codeVerifier) return NextResponse.redirect(new URL('/connect/canva?oauth=error&reason=invalid_state', appBaseUrl()));
  ...
  const tokenResp = await fetch('https://api.canva.com/rest/v1/oauth/token', {
    ...
    body: new URLSearchParams({ code, redirect_uri: new URL('/api/connect/canva/callback', appBaseUrl()).toString(), grant_type: 'authorization_code', code_verifier: codeVerifier }),
  });
  if (!tokenResp.ok) return NextResponse.redirect(new URL('/connect/canva?oauth=error&reason=token_exchange_failed', appBaseUrl()));
  ...
  return NextResponse.redirect(new URL('/connect/canva?oauth=success', appBaseUrl()));
}
