...
export async function GET(request: Request) {
  ...
  await Promise.all([
    supabase.from('oauth_tokens').upsert({
      ...
      scope: 'default', created_at: now, updated_at: now,
    }, { onConflict: 'user_id,platform' }),
    supabase.from('sync_status').upsert({
      user_id: userId, platform: 'asana', status: 'idle', sync_progress: 0, total_items: 0, updated_at: now,
    }, { onConflict: 'user_id,platform' }),
  ]);
