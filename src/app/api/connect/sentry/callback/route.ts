...
export async function GET(request: Request) {
  ...
  if (!clientId || !clientSecret) return NextResponse.redirect(new URL('/connect/sentry?oauth=error&reason=missing_env', url.origin));
  const cookieStore = await cookies();
  const expectedState = cookieStore.get('sentry_oauth_state')?.value;
  if (!expectedState || expectedState !== state || !code) return NextResponse.redirect(new URL('/connect/sentry?oauth=error&reason=invalid_state', url.origin));
  ...
  const tokenResp = await fetch('https://sentry.io/oauth/token/', {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: new URL('/api/connect/sentry/callback', url.origin).toString(),
      grant_type: 'authorization_code',
    }),
  });
  if (!tokenResp.ok) return NextResponse.redirect(new URL('/connect/sentry?oauth=error&reason=token_exchange_failed', url.origin));
  ...
  return NextResponse.redirect(new URL('/connect/sentry?oauth=success', url.origin));
}
