...
export async function GET(request: Request) {
  ...
  if (!clientId || !clientSecret) return NextResponse.redirect(new URL('/connect/fitbit?oauth=error&reason=missing_env', url.origin));
  ...
  if (!expectedState || expectedState !== state || !code || !codeVerifier) return NextResponse.redirect(new URL('/connect/fitbit?oauth=error&reason=invalid_state', url.origin));
  ...
  const tokenResp = await fetch('https://api.fitbit.com/oauth2/token', {
    ...
    body: new URLSearchParams({ code, redirect_uri: new URL('/api/connect/fitbit/callback', url.origin).toString(), grant_type: 'authorization_code', code_verifier: codeVerifier }),
  });
  if (!tokenResp.ok) return NextResponse.redirect(new URL('/connect/fitbit?oauth=error&reason=token_exchange_failed', url.origin));
  ...
  return NextResponse.redirect(new URL('/connect/fitbit?oauth=success', url.origin));
}
